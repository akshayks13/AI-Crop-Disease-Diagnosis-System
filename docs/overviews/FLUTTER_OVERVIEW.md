...
*   **`geolocator`** (v13.0.0): Gets the user's GPS coordinates for accurate market prices and weather data.
...
*   **`speech_to_text`** (v7.3.0): Allows farmers to ask questions via voice.
...
### What Exists
*   **`intl`** package (v0.19.0): Present but not actively used for UI text.
*   **`flutter_tts`**: Reads text aloud using device TTS (supports multiple languages via device settings).
*   **`speech_to_text`**: Recognizes speech in multiple languages.

### How to Add Full UI Translation
To support multiple languages (Hindi, Tamil, etc.):
...
2.  **Expert Q&A**: Ask questions, rate answers
3.  **Community Forum**: Posts, comments, likes
4.  **Farm Management**: Track crops, manage tasks
5.  **Market Prices**: Real-time commodity prices
6.  **Encyclopedia**: Browse crop and disease info
7.  **Accessibility**: TTS & STT for voice interaction
